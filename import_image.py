#this file is from here: https://github.com/JoseCelis/cloud-I
...
def main():
    ...
    for bbox_dict in tqdm(bbox_dicts):
        ...
        request_image_and_mask_to_sentinel2_L2A(bbox,
                                                bbox_dict["date"],
                                                sh_config,
                                                data_folder=images_folder)
    logging.info("Extracting .tar files . . .")
    [extract_tar_file("response.tar",
                      os.path.join(images_folder, path)) for path in os.listdir(images_folder)]
